...
def _buildpacket( first, region, filters ):
    ...
    packet = "1%c%s\0%s\0" % ( region, first, filters )
    print "packet: ",packet
...
if __name__ == "__main__":
    ...
    for server in query(gamedir='insurgency'):
        print server
    print "done"
    print
"""
    print "Fetching Europe's Counter-Strike servers playing de_dust2..."
    insurgency = set( query( region=EUROPE, gamedir="insurgency" ) )
    print "done, length =", len( insurgency )
