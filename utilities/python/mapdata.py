...
require_once "{$includepath}/classes/KeyValues.php";
...
function DoMod($mod="insurgency", $version="latest") {
	global $mapfilter, $force;
	if (is_numeric($mod)) {
		$mod = $version;
		$version = "latest";
	}
...
//echo "loading cpsetup\n";
	$data = parseKeyValues(file_get_contents($srcfiles["CPSetup"]));
var_dump($data);
...
//echo "merging {$base}\n";
				...
				$basedata = parseKeyValues(file_get_contents(GetDataFile("maps/{$base}")));
				var_dump($basedata);
